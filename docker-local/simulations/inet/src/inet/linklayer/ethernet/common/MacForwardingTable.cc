...
void MacForwardingTable::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        ofstream macFile;
        macFile.open ("MacTableEntries.txt");
        macFile <<"VLAN ID    MAC    IfId    Inserted" << endl;
        macFile.close();
...
void MacForwardingTable::readManualForwardingTable(const char *fileName)
{
    FILE *fp = fopen(fileName, "r");
    if (fp == nullptr)
        throw cRuntimeError("cannot open address table file `%s'", fileName);
...
void MacForwardingTable::writeTableToFile(const char* filename) const {
    std::ofstream out(filename, std::ios::app);

    if (!out.is_open()) {
        std::cout << "Failed to open file for writing MAC table: " << filename << endl;
        return;
    }

    out << ">>" << this->getFullPath() <<"\n";
    for (const auto& x : forwardingTable) {
        ForwardingTableKey key = x.first; //(vlanId, macAddress);
        AddressEntry entry = x.second; //vlanId, interfaceId, 0
        out << key.first << " " << key.second.str() << " " << entry.interfaceId << " " << entry.insertionTime.dbl() << "\n";
    }
...
MacForwardingTable::~MacForwardingTable(){
    writeTableToFile("MacTableEntries.txt");
}
} // namespace inet
